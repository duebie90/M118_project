...
unsigned int sf_process_check_sum_call( int nlhs, mxArray * plhs[], int nrhs,
  const mxArray * prhs[] )
{
  extern unsigned int sf_MiL_BMS_Bat_process_check_sum_call( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_MiL_BMS_Bat_process_check_sum_call(nlhs,plhs,nrhs,prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_MiL_BMS_Bat_autoinheritance_info( int nlhs, mxArray *
    plhs[], int nrhs, const mxArray * prhs[] );
  char commandName[64];
  ...
  if (strcmp(machineName, "MiL_BMS_Bat") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_MiL_BMS_Bat_autoinheritance_info(nlhs,plhs,3,newRhs);
  }
...
unsigned int sf_process_get_eml_resolved_functions_info_call( int nlhs, mxArray *
  plhs[], int nrhs, const mxArray * prhs[] )
{
  extern unsigned int sf_MiL_BMS_Bat_get_eml_resolved_functions_info( int nlhs,
  ...
  if (strcmp(machineName, "MiL_BMS_Bat") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_MiL_BMS_Bat_get_eml_resolved_functions_info(nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_MiL_BMS_Bat_method_dispatcher(SimStruct *S, unsigned int
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"MiL_BMS_Bat")) {
    return(sf_MiL_BMS_Bat_method_dispatcher(simstructPtr,chartFileNumber,
...
extern void MiL_BMS_Bat_terminator(void);
...
extern void MiL_BMS_Bat_initializer(void);
extern void MiL_BMS_Bat_register_exported_symbols(SimStruct* S);
extern void MiL_BMS_Bat_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  MiL_BMS_Bat_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      MiL_BMS_Bat_debug_initialize();
    }

    MiL_BMS_Bat_initializer();
  }
